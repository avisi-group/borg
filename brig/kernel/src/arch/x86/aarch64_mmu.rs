...
pub fn guest_translate(device: &ModelDevice, guest_virtual_address: u64) -> Option<u64> {
    let tcr_el1 = device.register_file.lock().read::<u64, _>("TCR_EL1_bits");
    log::trace!("tcr_el1: {tcr_el1:x}");
    // let ttbcr = device.register_file.lock().read::<u32>("TTBCR_S_bits");
    // log::trace!("{ttbcr:032b}");
    // let ttbcr_n = ttbcr & 0b111;
    // exit_with_message!("{ttbcr_n:03b}");

    let ttbr0_el1 = device
        .register_file
        .lock()
        .read::<u64, _>("_TTBR0_EL1_bits");
    let ttbr1_el1 = device
        .register_file
        .lock()
        .read::<u64, _>("_TTBR1_EL1_bits");
...
fn guest_page_fault(device: &ModelDevice, guest_virtual_address: u64) {
    ...
    let retaddr = device.register_file.lock().read::<u64, _>("_PC");
...
fn take_arm_exception(
    ...
    let current_el = device.register_file.lock().read::<u8, _>("PSTATE_EL");
    log::trace!("current_el: {current_el}");

    if target_el > current_el {
        voff += 0x400;
    } else if device.register_file.lock().read::<u8, _>("PSTATE_SP") == 1 {
        voff += 0x200;
    }
    ...
    device
        .register_file
        .lock()
        .write::<u8, _>("PSTATE_EL", target_el);

    // Update spsel
    device.register_file.lock().write::<u8, _>("PSTATE_SP", 1);

    if target_el == 1 {
        device
            .register_file
            .lock()
            .write::<u32, _>("SPSR_EL1_bits", spsr);
        device
            .register_file
            .lock()
            .write::<u64, _>("ELR_EL1", retaddr);

        // If it's NOT an IRQ...
        if typ != 255 {
            let ec = get_exception_class(current_el, target_el, typ);
            device.register_file.lock().write::<u32, _>(
                "ESR_EL1_bits",
    ...
    device.register_file.lock().write::<u8, _>("PSTATE_D", 1);
    device.register_file.lock().write::<u8, _>("PSTATE_A", 1);
    device.register_file.lock().write::<u8, _>("PSTATE_I", 1);
    device.register_file.lock().write::<u8, _>("PSTATE_F", 1);

    let vbar = device
        .register_file
        .lock()
        .read::<u64, _>(match current_el {
            1 => "VBAR_EL1",
            2 => "VBAR_EL2",
            3 => "VBAR_EL3",
    ...
    device
        .register_file
        .lock()
        .write::<u64, _>("_PC", vbar + voff);
}

fn get_psr_from_pstate(device: &ModelDevice) -> u32 {
    let n = device.register_file.lock().read::<u8, _>("PSTATE_N") as u32;
    let z = device.register_file.lock().read::<u8, _>("PSTATE_Z") as u32;
    let c = device.register_file.lock().read::<u8, _>("PSTATE_C") as u32;
    let v = device.register_file.lock().read::<u8, _>("PSTATE_V") as u32;
    let d = device.register_file.lock().read::<u8, _>("PSTATE_D") as u32;
    let a = device.register_file.lock().read::<u8, _>("PSTATE_A") as u32;
    let i = device.register_file.lock().read::<u8, _>("PSTATE_I") as u32;
    let f = device.register_file.lock().read::<u8, _>("PSTATE_F") as u32;
    let el = device.register_file.lock().read::<u8, _>("PSTATE_EL") as u32;
    let sp = device.register_file.lock().read::<u8, _>("PSTATE_SP") as u32;
