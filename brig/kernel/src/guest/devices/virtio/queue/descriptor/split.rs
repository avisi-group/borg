...
pub struct Descriptor {
    ...
    /// Index into the descriptor table of the next descriptor if flags has the
    /// `next` bit set.
    next: u16,
}
...
impl Descriptor {
    ...
    /// Check whether this descriptor refers to a buffer containing an indirect
    /// descriptor table.
...
impl VirtqUsedElem {
    ...
    /// * `len` - the total length of the descriptor chain which was used
    ///   (written to).
    #[allow(unused)]
...
// // SAFETY: This is safe because `VirtqUsedElem` contains only wrappers over
// POD // types and all accesses through safe `vm-memory` API will validate any
// garbage // that could be included in there.
