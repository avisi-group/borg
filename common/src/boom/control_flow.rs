...
use {
    crate::{
        boom::{Expression, Literal, Statement, Value},
        id::Id,
...
impl Terminator {
    ...
    pub fn targets(&self) -> Vec<ControlFlowBlock> {
        match self {
            Terminator::Return(_) | Terminator::Panic(_) => alloc::vec![],
            Terminator::Conditional {
                target,
                fallthrough,
                ..
            } => alloc::vec![target.clone(), fallthrough.clone()],
            Terminator::Unconditional { target } => alloc::vec![target.clone()],
        }
    }
}
